...
class PipeWireCapture(AudioCapture):
    ...
    def start(self) -> None:
        ...
        target = _extract_pipewire_target(self.source_id)
        if target:
        ...
        try:
            ...
            raise RuntimeError("pw-cat is not available") from exc
    ...
    def read(self) -> AudioFrame:
        if self._proc is None or self._proc.stdout is None:
            raise RuntimeError("PipeWire capture is not started")
        ...
        return AudioFrame(
            ...
            t0_ms=t0,
            t1_ms=t1,
            source_id=self.source_id or "pipewire",
        )


def _extract_pipewire_target(source_id: str) -> str:
    if source_id.startswith("pw:"):
